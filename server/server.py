...
async def handle_client(ws):#(ws, path)
    ...
    try:
        ...
        if room:
            ...
            # in case the room is empty, we can cancel the processing task
            # to free up resources

            # if len(room.players) == 0 and hasattr(room, 'processing_task'):
            #     room.processing_task.cancel()
            #     print(f"Room {room.room_id} processing task cancelled")
            room_manager.cleanup_empty_room(room.room_id)
